...
def crossvalidation_activation():
	...
	for activation in {'identity', 'logistic', 'tanh', 'relu'}:
		clf = MLPClassifier(random_state=0, hidden_layer_sizes = (12, 9), activation = activation)
		clf = clf.fit(x_train, y_train)
		cv_result = cross_validate(clf, X, Y, cv = 3, return_train_score=False)
		mean = np.mean(cv_result['test_score'])
...
def neuralnet_layers():
	...
	for i in range(1,95):
	    ...
	    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(12, 10, 5), random_state=1, activation='logistic')
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size=1 - i / 100)
	    clf.fit(X_train, y_train)
	    list3.append(accuracy_score(y_train, clf.predict(X_train)))
	    list4.append(accuracy_score(y_test, clf.predict(X_test)))
	    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(12, 9), random_state=1, activation='logistic')
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size=1 - i / 100)
	    clf.fit(X_train, y_train)
	    ...
	    print(i, ' iteration')
	plt.plot(range(len(list1)),list1, '-b', label="Training Set(12,10,7,5,3) logistic")
	plt.plot(range(len(list2)),list2, '-b', label="Testing Set(12,10,7,5,3) logistic")
	plt.plot(range(len(list3)),list3, '-r', label="Training Set(12,10,5) logistic")
	plt.plot(range(len(list4)),list4, '-r', label="Testing Set(12,10,5) logistic")
	plt.plot(range(len(list5)),list5, '-y', label="Training Set(12,9) logistic")
	plt.plot(range(len(list6)),list6, '-y', label="Testing Set(12,9) logistic")
	plt.plot(range(len(list7)),list7, '-g', label="Training Set(10) logistic")
	plt.plot(range(len(list8)),list8, '-g', label="Testing Set(10) logistic")
	plt.legend(loc="best")
...
def neuralnet_activation():
	...
	list8=[]
	for i in range(1,95):
	    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(12, 9), random_state=1, activation='relu')
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size=1 - i / 100)
	    clf.fit(X_train, y_train)
	    list1.append(accuracy_score(y_train, clf.predict(X_train)))
	    list2.append(accuracy_score(y_test, clf.predict(X_test)))
	    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(12, 9), random_state=1, activation='identity')
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size=1 - i / 100)
	    clf.fit(X_train, y_train)
	    list3.append(accuracy_score(y_train, clf.predict(X_train)))
	    list4.append(accuracy_score(y_test, clf.predict(X_test)))
	    ...
	    list5.append(accuracy_score(y_train, clf.predict(X_train)))
	    list6.append(accuracy_score(y_test, clf.predict(X_test)))
	    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(12, 9), random_state=1, activation='tanh')
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size=1 - i / 100)
	    clf.fit(X_train, y_train)
	    list7.append(accuracy_score(y_train, clf.predict(X_train)))
	    list8.append(accuracy_score(y_test, clf.predict(X_test)))
	...
	plt.legend(loc="best")
	plt.title('Neural Net with 2 hidden layers(12, 9)')
	plt.ylabel('Accuracy Rate')
	plt.xlabel('Train data size')
	plt.show()
