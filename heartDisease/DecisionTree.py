...
def result():
	clf = DecisionTreeClassifier(random_state=0, criterion='entropy', max_depth=5, max_leaf_nodes = 7)	
	clf.fit(X_train, y_train)
	# print(i + 1, ': ', np.mean(cross_validate(clf, X, Y, cv = 5, return_train_score=True)['test_score']))
...
def wineQuality():
	...
	list8=[]
	for i in range(1,95):
	    clf = DecisionTreeClassifier(random_state=0, criterion='entropy', max_leaf_nodes=2)#, max_leaf_nodes = 5)
	    ...
	    clf = DecisionTreeClassifier(random_state=0, criterion='entropy', max_leaf_nodes=3)#, max_leaf_nodes = 5)
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size = 1 - i / 100)
	    clf = clf.fit(X_train, y_train) #After being fitted, the model can then be used to predict the class of samples
	    list3.append(accuracy_score(y_train, clf.predict(X_train)))
	    list4.append(accuracy_score(y_test, clf.predict(X_test)))

	    clf = DecisionTreeClassifier(random_state=0, criterion='entropy', max_leaf_nodes=5)#, max_leaf_nodes = 5)
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size = 1 - i / 100)
	    clf = clf.fit(X_train, y_train) #After being fitted, the model can then be used to predict the class of samples
	    list5.append(accuracy_score(y_train, clf.predict(X_train)))
	    list6.append(accuracy_score(y_test, clf.predict(X_test)))

	    clf = DecisionTreeClassifier(random_state=0, criterion='entropy', max_leaf_nodes=7)#, max_leaf_nodes = 5)
	    X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size = 1 - i / 100)
	    clf = clf.fit(X_train, y_train) #After being fitted, the model can then be used to predict the class of samples
	    list7.append(accuracy_score(y_train, clf.predict(X_train)))
	    list8.append(accuracy_score(y_test, clf.predict(X_test)))
	plt.plot(range(len(list1)),list1, '-b', label="Training Set max_leaf_nodes = 2")
	plt.plot(range(len(list2)),list2, '-b', label="Testing Set, max_leaf_nodes = 2")
	plt.plot(range(len(list3)),list3, '-r', label="Training Set max_leaf_nodes = 3")
	plt.plot(range(len(list4)),list4, '-r', label="Testing Set, max_leaf_nodes = 3")
	plt.plot(range(len(list5)),list5, '-y', label="Training Set max_leaf_nodes = 5")
	plt.plot(range(len(list6)),list6, '-y', label="Testing Set, max_leaf_nodes = 5")
	plt.plot(range(len(list7)),list7, '-g', label="Training Set max_leaf_nodes = 7")
	plt.plot(range(len(list8)),list8, '-g', label="Testing Set, max_leaf_nodes = 7")
	plt.legend(loc="best")
	plt.title('Decision Tree with different max_leaf_nodes')
