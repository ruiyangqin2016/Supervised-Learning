...
def crossvalidation_learningRate():
	LR = 0.01
	scoreList = []
	while LR <= 1:
		...
		cv_result = cross_validate(clf, X, Y, cv = 3, return_train_score=False)
		mean = np.mean(cv_result['test_score'])
		print('finish ', (int)(LR * 100), 'mean is ', mean)
		scoreList.append(mean)
		LR = LR + 0.01
	plt.plot(range(len(scoreList)), scoreList, '-r', label='cross_validation')
	plt.legend(loc='best')
	plt.title('cross validation of learning_rate with n_estimators is 10')
	plt.ylabel('mean test score')
	plt.xlabel('learning_rate')
	plt.show()
# crossvalidation_learningRate()
...
def crossvalidation_nEstimators():
	...
	plt.plot(range(len(scoreList)), scoreList, '-r', label='cross_validation')
	plt.legend(loc='best')
	plt.title('cross validation of n_estimators given base_estimator max_depth = 7')
	plt.ylabel('mean test score')
...
def boost():
	...
	for i in range(1,95):
		...
		list7.append(accuracy_score(y_train, train_predict))
		list8.append(accuracy_score(y_test, test_predict))
	plt.plot(range(len(list1)),list1, '-r', label="Training Set(base_estimator MD = 2)")
	plt.plot(range(len(list2)),list2, '-r', label="Testing Set(base_estimator MD = 2)")
	plt.plot(range(len(list7)),list7, '-g', label="Training Set(base_estimator MD = 5)")
	plt.plot(range(len(list8)),list8, '-g', label="Testing Set(base_estimator MD = 5)")
	...
	plt.plot(range(len(list5)),list5, '-y', label="Training Set(base_estimator MD = 10)")
	plt.plot(range(len(list6)),list6, '-y', label="Testing Set(base_estimator MD = 10)")
	plt.legend(loc="best")
...
base_estimator = DecisionTreeClassifier(random_state=0, criterion='entropy', max_depth=10)
clf = AdaBoostClassifier(base_estimator = base_estimator, n_estimators = 55)
X_train, X_test, y_train, y_test = train_test_split(X, Y, random_state=0, test_size=0.25)
clf.fit(X_train, y_train)
print("The prediction accuracy of boosting is " + str(accuracy_score(y_test, clf.predict(X_test))))